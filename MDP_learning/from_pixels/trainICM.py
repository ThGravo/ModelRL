...
def train_icm(cfg, dqn, nb_actions, do_load_dqn_weights=True):
    ...
    im_model.compile(optimizer='nadam', loss=['sparse_categorical_crossentropy', custom_loss],
                     metrics=['sparse_categorical_accuracy', mape, custom_metric], loss_weights=[.1, 100])
    ...
    log_string = 'ICM_{}_nb_actions{}_loadDQNwgths{}-{}'.format(cfg.env_name, nb_actions, do_load_dqn_weights,
                                                                time.time())
    print('logging to {}'.format(log_string))
    ####################################################################################################################

    data_size = dqn.memory.observations.length
    chunk_size = max(min(data_size - 5, 100000), int(data_size / 100))
    ...
    for ii in range(n_rounds):
        ...
        states = np.empty((chunk_size,) + state_size, dtype=np.float32)
        next_state = np.empty((chunk_size,) + state_size, dtype=np.float32)
        actions = np.empty((chunk_size, nb_actions), dtype=np.float32)
        rewards = np.empty((chunk_size, 1), dtype=np.float32)
        terminals = np.empty((chunk_size, 1), dtype=np.float32)
        empty_hstates = np.empty((chunk_size, encoded_0._keras_shape[1]), dtype=np.float32)
        ...
        im_model.fit([state0_seq, state1_seq, actions_encoded], [action_seq, hstate1_seq],
                ...
                TensorBoard(log_dir='./invmod_logs/{}/{}'.format(os.path.splitext(cfg.filename)[0], log_string))])
    return im_model
    # #######################################################################################################################
